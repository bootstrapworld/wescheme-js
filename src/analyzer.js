...
defFunc.prototype.desugar = function(pinfo){
  ...
  var newDefFunc = new defFunc(this.name, this.args, bodyAndPinfo[0], this.stx);
...
defVar.prototype.desugar = function(pinfo){
  ...
  if(this.expr instanceof lambdaExpr){
    ...
    var newDefVar = new defVar(this.name, exprAndPinfo[0], this.stx);
...
defStruct.prototype.desugar = function(pinfo){
  ...
  var idSymbols = ids.map(function(id){return new symbolExpr(id);});
  var makeStructTypeFunc = new symbolExpr('make-struct-type');
  var makeStructTypeArgs = [
    new quotedExpr(new symbolExpr(this.name.val)),
    new literal(false),
  ...
  function makeAccessorDefn(f, i){
    ...
    var makeFieldArgs = [new symbolExpr(that.name.val+'-ref'), new literal(i), new quotedExpr(new symbolExpr(f.val))];
    var makeFieldCall = new callExpr(makeFieldFunc, makeFieldArgs);
    var accessorSymbol= new symbolExpr(that.name.val+'-'+f.val);
...
beginExpr.prototype.desugar = function(pinfo){
  ...
  var newBeginExpr = new beginExpr(exprsAndPinfo[0], this.stx);
...
lambdaExpr.prototype.desugar = function(pinfo){
  ...
  var bodyAndPinfo = this.body.desugar(pinfo);
  var newLambdaExpr = new lambdaExpr(this.args, bodyAndPinfo[0], this.stx);
...
localExpr.prototype.desugar = function(pinfo){
  ...
  var newLocalExpr = new localExpr(defnsAndPinfo[0], exprAndPinfo[0], this.stx);
...
ifExpr.prototype.desugar = function(pinfo){
  ...
  var newIfExpr = new ifExpr(predicate, consequence, alternative, this.stx);
...
whenUnlessExpr.prototype.desugar = function(pinfo){
  ...
  var call_exp = new callExpr(void_exp, [], this.stx);
  ...
  var exprsAndPinfo = desugarProgram([this.predicate,
    ...
    alternative],
    pinfo);
  var if_exp = new ifExpr(exprsAndPinfo[0][0], exprsAndPinfo[0][1], exprsAndPinfo[0][2], this.stx);
...
letExpr.prototype.desugar = function(pinfo){
  ...
  var lambda= new lambdaExpr(ids, this.body, this.stx);
...
condExpr.prototype.desugar = function(pinfo){
  ...
  var exhaustedLoc = new quotedExpr(new literal(this.location.toVector()));
  var expr = tagApplicationOperator_Module(new callExpr(condExhausted, [exhaustedLoc])
...
caseExpr.prototype.desugar = function(pinfo){
  ...
  function processClause(base, clause){
    ...
    var callStx = new callExpr(ormapStx, [predicateStx, clause.first], that.stx);
...
andExpr.prototype.desugar = function(pinfo){
  ...
  function desugarAndExprs(exprs){
    ...
    var consequence = (exprs.length > 2)? desugarAndExprs(exprs.slice(1))
...
orExpr.prototype.desugar = function(pinfo){
  ...
  function desugarOrExprs(exprs, pinfo){
    ...
    var tmpBinding = new couple(firstExprSym, forceBooleanContext(that.stx, that.stx.location, firstExpr));
    var secondExpr;
...
function desugarQuasiQuotedList(qqlist, pinfo, depth) {
  ...
  var appendArgs = qqlist.map(function(x){ return desugarQuasiQuotedListElement(x, pinfo, depth, loc)[0]; });
...
symbolExpr.prototype.desugar = function(pinfo){
  ...
  if(this.val === "."){
    let msg = new types.Message([this.location.source, ":",
...
unsupportedExpr.prototype.desugar = function(unusedPinfo){
...
defVars.prototype.collectDefinitions = function(pinfo){
  var that = this;
  var fieldToAccessor = function(f){return that.stx[1].val+"-"+f.val;};
  var fieldToMutator = function(f){return "set-"+that.stx[1].val+"-"+f.val+"!";};
  ...
  if(window.COLLECTIONS && window.COLLECTIONS[moduleName]){
    ...
    if (window.jQuery) {
      window.jQuery.ajax({
        ...
        error: function (unusedError) { throwModuleError(moduleName); },
        async: false
      });
    } else {
...
// BINDING STRUCTS ///////////////////////////////////////////////////////
export function provideBindingId(symbl){ this.symbl = symbl;}
export function provideBindingStructId(symbl){ this.symbl = symbl; }
...
Program.prototype.analyzeUses = function(pinfo, unusedEnv){ return pinfo; };
...
lambdaExpr.prototype.analyzeUses = function(pinfo, unusedEnv){
  return analyzeClosureUses(this, pinfo);
};
...
symbolExpr.prototype.analyzeUses = function(pinfo, env){
  ...
  if(structures.keywords.indexOf(this.val) > -1 &&
    ...
    throwError(
      new types.Message([
        new types.ColoredPart(this.val, this.location),
        ": expected an open parenthesis before ",
        this.val,
        ", but found none"
      ]),
...
export var analyze = function(program, debug){
  var start       = new Date().getTime();
  try { var pinfo       = _analyze(program); }             // do the actual work
