...
// Function definition
export function defFunc(name, args, body, stx) {
  Program.call(this);
  this.name = name;
  this.args = args;
  this.body = body;
...
// Variable definition
export function defVar(name, expr, stx) {
  Program.call(this);
  this.name = name;
  this.expr = expr;
  this.stx  = stx;
...
export function defVars(names, expr, stx) {
  Program.call(this);
  this.names  = names;
  this.expr   = expr;
  this.stx    = stx;
...
export function defStruct(name, fields, stx) {
  Program.call(this);
  this.name   = name;
  this.fields = fields;
  this.stx    = stx;
...
// Begin expression
export function beginExpr(exprs, stx) {
  Program.call(this);
  this.exprs  = exprs;
  this.stx    = stx;
  this.toString = function(){
...
// Lambda expression
export function lambdaExpr(args, body, stx) {
  Program.call(this);
  this.args = args;
  this.body = body;
  this.stx  = stx;
...
// Local expression
export function localExpr(defs, body, stx) {
  Program.call(this);
  this.defs = defs;
  this.body = body;
  this.stx  = stx;
...
// Letrec expression
export function letrecExpr(bindings, body, stx) {
...
// and expression
export function andExpr(exprs, stx) {
  this.exprs  = exprs;
  this.stx    = stx;
...
// if expression
export function ifExpr(predicate, consequence, alternative, stx) {
  Program.call(this);
...
export function whenUnlessExpr(predicate, exprs, stx) {
  Program.call(this);
  this.predicate = predicate;
  this.exprs = exprs;
  this.stx = stx;
...
export function symbolExpr(val, stx) {
  Program.call(this);
  this.val = val;
  this.stx = stx;
}
...
// quoted expression
export function quotedExpr(val) {
  Program.call(this);
  this.val = val;
  this.toString = function() {
...
// unquoted expression
export function unquotedExpr(val) {
  Program.call(this);
  this.val = val;
  this.toString = function(){ return ","+this.val.toString(); };
}
...
export function quasiquotedExpr(val) {
  Program.call(this);
  this.val = val;
  this.toString = function(){
...
// unquote-splicing
export function unquoteSplice(val) {
  Program.call(this);
  this.val = val;
  this.toString = function(){ return ",@"+this.val.toString();};
}
...
// require expression
export function requireExpr(spec, stx) {
  Program.call(this);
  this.spec = spec;
  this.stx  = stx;
...
// provide expression
export function provideStatement(clauses, stx) {
  Program.call(this);
  this.clauses  = clauses;
  this.stx      = stx;
...
export function unsupportedExpr(val, errorMsg, errorSpan) {
  Program.call(this);
  this.val = val;
...
export function unnamedEnv(parent){
  env.call(this);
  this.parent = parent;
...
export function localEnv(name, boxed, parent){
  env.call(this);
  this.name   = name;
  this.boxed  = boxed;
  this.parent = parent;
  this.lookup = function(name, depth){
    ...
    // TODO: fix this circular dependency
    var compiler = require('./compiler')
    return (name===this.name)? new compiler.localStackReference(name, this.boxed, depth)
    : this.parent.lookup(name, depth+1);
  };
}
localEnv.prototype = heir(env.prototype);

export function globalEnv(names, boxed, parent){
  env.call(this);
  this.names  = names;
  this.boxed  = boxed;
  this.parent = parent;
...
export var moduleGuess = function(wrongName){
  // TODO: fix this circular dependency
  var modules = require('./modules')
...
export function pinfo(env, modules, usedBindingsHash, freeVariables, gensymCounter,
