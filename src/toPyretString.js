...
(function() {
  ...
  function converttoPyretString(programs, pinfo) {
    _pinfo = pinfo;
    // identify structs
    var isStruct = function(b) {
        return (b instanceof structBinding);
      },
      accumulateStructInfo = function(b) {
        constructors[b.constructor] = b.name;
        predicates[b.predicate] = "is-" + b.name;
        b.accessors.forEach(function(a) {
          accessors[a] = a.substring(b.name.length + 1);
        });
      },
      isTestCase = function(p) {
        return (p instanceof callExpr) && (p.func instanceof symbolExpr) && (["check-expect", "EXAMPLE", "check-within"].indexOf(p.func.val) > -1);
      },
      defsAndExprs = programs.filter(function(p) {
        return !isTestCase(p);
      }),
  ...
  function toPyretString(p) {
    return p.toPyretString();
  }
  ...
  function makeStructFromMembers(constructor, elts, isQuoted) {
    var args = elts.map(function(e) {
      return (e instanceof Array) ? makeStructFromMembers(constructor, e, isQuoted) : (isQuoted && e instanceof symbolExpr) ? makeLiteralFromSymbol(e) : e.toPyretString();
    });
  ...
  defStruct.prototype.toPyretString = function() {
    ...
    str += "fun make-" + name + "(" + this.fields.map(toPyretString).join(",") + "): " + name + "(" + this.fields.map(toPyretString).join(",") + ") end\n";
    return str;
  };
  ...
  whenUnlessExpr.prototype.toPyretString = function() {
    ...
    var pred = (this.stx.val === "unless") ? "not(" + this.predicate.toPyretString() + ")" : this.predicate.toPyretString(),
  ...
  letExpr.prototype.toPyretString = function() {
    // if there are no defs, just translate the body
    if (this.bindings.length === 0) return this.body.toPyretString();
    var tmpIDs = this.bindings.map(function(c) {
        return c.first.toPyretString() + "_tmp";
      }),
      // bind the rhs to lhs_tmp (a_1 = 5, ...)
      tmpBindings = this.bindings.map(function(c, i) {
        return tmpIDs[i] + " = " + c.second.toPyretString();
      }),
      // bind lhs_tmp to lhs (a = a_1, ...)
      newBindings = this.bindings.map(function(c, i) {
        return c.first.toPyretString() + " = " + tmpIDs[i];
      });
  ...
  caseExpr.prototype.toPyretString = function() {
    function convertClause(c) {
      return "| any(lam(elt): equal-always(_val,elt) end, " + c.first.toPyretString() + ") then: " + c.second.toPyretString();
    }
    return "block: \n _val=" + this.expr.toPyretString() + "\n ask:" + this.clauses.map(convertClause).join("\n") + "\n end \n end";
  };
  ...
  condExpr.prototype.toPyretString = function() {
    function convertClause(couple) {
      return " | " + ((couple.first.toPyretString() == "else") ? " otherwise: " : couple.first.toPyretString() + " then: ") + couple.second.toPyretString() + "\n";
    }
  ...
  ifExpr.prototype.toPyretString = function() {
    var rawPredicate = ((this.predicate instanceof callExpr) && (this.predicate.func.val === "verify-boolean-branch-value")) ?
  ...
  symbolExpr.prototype.toPyretString = function() {
    return (symbolMap[this.val]) ? symbolMap[this.val] : this.val.length === 1 ? this.val : this.val.replace(/\//g, 'SLASH').replace(/\?/g, 'QUESTION').replace(/\!/g, 'BANG').replace(/\+/g, 'PLUS');
  };

  // literals
  // literal(String|Char|Number)
  literal.prototype.toPyretString = function() {
    return this.val.toPyretString ? this.val.toPyretString() : this.val.toWrittenString ? this.val.toWrittenString()
      /* else */
      : this.val.toString();
  };
